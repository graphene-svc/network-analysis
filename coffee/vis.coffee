...
Network = () ->
  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d.position))
  ...
  setupData = (data) ->
    ...
    countExtent = d3.extent(data.nodes, (d) -> d.eventcount)
    ...
    data.nodes.forEach (n) ->
      ...
      n.radius = circleRadius(n.eventcount)

    # id's -> node objects
    nodesMap  = mapNodes(data.nodes)

    # switch links to point to node objects instead of id's
    countExtentForCommonEvent = d3.extent(data.links, (d) -> d.commoneventcount)
  ...
  # attr is value stored in node, like 'position'
  ...
  popularityNodes = (allNodes) ->
    filteredNodes = allNodes
    if filter == "popular" or filter == "obscure"
      eventcounts = allNodes.map((d) -> d.eventcount).sort(d3.ascending)
  ...
  # Returns array of positions sorted based on
  # current sorting method.
  sortedPositions = (nodes,links) ->
    positions = []
    if sort == "all"
      counts = {}
      links.forEach (l) ->
        counts[l.source.position] ?= 0
        counts[l.source.position] += 1
        counts[l.target.position] ?= 0
        counts[l.target.position] += 1
      # add any missing positions that dont have any links
      nodes.forEach (n) ->
        counts[n.position] ?= 0

      # sort based on counts
      positions = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      # get just names
      positions = positions.map (v) -> v.key
  ...
  updateNodes = () ->
    ...
    node.enter().append("circle")
      ...
      .attr("cy", (d) -> d.y)
      .attr("r", (d) -> d.radius)
      .style("fill", (d) -> nodeColors(d.position))
  ...
  updateLinks = () ->
    ...
    link.enter().append("line")
      ...
      .style("stroke-width", (d) -> d.weightage)

    linkLabel = linksG.selectAll(".linkLabel")
      .data(curLinksData, (d) -> "#{d.source.id}_#{d.target.id}")
  ...
  moveToRadialLayout = (alpha) ->
    k = alpha * 0.1
    (d) ->
      centerNode = groupCenters(d.position)
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.position)).darker().toString()
